...
server.tool(
    ...
    "Generates synthetic patient records (FHIR R4) using the Synthea Java CLI. Useful for testing applications with realistic healthcare data.",
    {
...
/**
 * Tool: list_generated_patients
 * Allows clients to read the list of generated synthetic JSON files.
 */
server.tool(
    "list_generated_patients",
    "List all generated synthetic patient FHIR JSON files in the output directory",
    {},
    async () => {
        try {
            ...
            return {
                content: [{
                    type: "text",
                    text: `No generated patients found or output directory does not exist. Error: ${error}`
                }],
                isError: true
            };
        }
